...
### 모달 상호작용
> 모바일 기기에서는 사용자가 뒤로 가기 버튼으로 모달 창을 닫는 경우가 많은데 이런 로직은 데스크탑과 사뭇 다릅니다.
- 모바일, 웹 모두에 적용시키기 위해 모달을 Link와 연결하여 URI 인식으로 모달을 여닫는 로직으로 변경했습니다.

### 문제 해결과 동시에 자원 절약
> dialog는 열리기 전에도 내부 컴포넌트를 렌더링하므로 리소스를 소모합니다. 이는 지도 API의 요청 낭비와 오동작까지 유발합니다.  
- 처음엔 상태관리로 dialog를 활성화, 비활성화하는 방식을 사용했지만 이는 dialog의 원활한 작동을 방해했습니다.
- 모바일 환경 특성상 모달을 뒤로 가기 버튼으로도 닫을 수 있어야 하는데 이에 대처를 못해 뒤로 가기로 페이지 이동 시 side effect가 발생했습니다.
- 상태로 모달을 여닫는 대신 라우터로 URI를 인식해서 모달을 여닫도록 변경하고, 모달이 열릴 때에만 내부 컴포넌트가 렌더링되도록 바꿔 해결했습니다.

### FCP 개선
> 기존에는 상세 페이지 탐색 시간이 3초를 초과했습니다. 이는 사용 경험을 매우 떨어뜨리는 요소입니다.
...
### 크로스 브라우징 대처 
> ResponsivelyApp을 사용함으로 여러 환경에서의 실행화면을 동시에 보고 문제를 빠르게 탐색합니다.  
- 일부 구형 iPhone에서 Cloudinary가 auto 설정으로 제공하는 이미지의 확장자를 로드하지 못하는 현상을 일찍 발견하고 수정했습니다.

# 학습 / 인사이트

### 트레이드 오프
개발 과정에서 비용과 이점을 비교할 때가 많았습니다.
...
- 타입스크립트 학습: 처음에는 프로젝트에 간단한 타이핑만 하되 타입 에러를 구글링해서 해결했습니다. 이런 임시 방편으론 타입스크립트를 쓰는 의미가 희미하다고 판단했고, 매주 목요일 밤마다 시간을 내 스터디를 하며 타입스크립트 코드를 리팩토링합니다.
