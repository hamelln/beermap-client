...
> 이에 대처를 안 해서 뒤로 가기로 페이지 이동 시 화면이 고정되거나 기능이 멈추는 side effect가 발생했습니다.  
> 게다가 dialog는 열리기 전에도 내부 컴포넌트를 렌더링하므로 리소스를 소모합니다.  
> 이는 지도 API 요청 낭비를 유발할 뿐더러, 지도 API가 dialog의 위치를 잘못 잡아서 비뚤어지게 렌더링하는 side effect도 발생합니다.  
...
### 크로스 브라우징 대처 
> ResponsivelyApp을 도입해서서 여러 환경에서의 실행화면을 동시에 보고 문제를 빠르게 탐색합니다.  
